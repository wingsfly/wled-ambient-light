...
// ── 实测标定表（**真实素材，不是合成的**）──
//
// 参数是扫出来的，判据是「语音与音乐要过、纯鼓要不过」：
//
//   共振峰下界/谐波门槛     中文语音        真实音乐        纯鼓(阴性)
//   300 / 0.35            0.35 (  9%)     0.36 (  0%)     0.00 ( 0%)   ← 初版，语音几乎不响
//   200 / 0.35            0.56 ( 57%)     0.47 (  1%)     0.00 ( 0%)
//   **200 / 0.25**        0.60 ( 69%)     0.49 (  7%)     0.00 ( 0%)   ← 现在用这组
//   150 / 0.15            0.67 ( 93%)     0.54 ( 44%)     0.00 ( 3%)   ← 音乐也开始乱响
//
//   （数字是 vocal 均值与「过 0.55 阈值」的帧占比。素材：macOS say 生成的
//    12 秒中文会议发言、30 秒真实音乐、12 秒合成鼓机。）
//
// 纯鼓在所有参数下都是 0 —— 打击声不会被误判成人声，这条很稳。
//
// **合成素材曾经把我带偏过。** 早期只用 tools/lamp_calib.cpp 里的合成信号
// 标定，那里的「纯人声」共振峰占比 0.997，于是我把门槛定在 0.62；
// 而真实音乐是 0.624、真实语音是 0.579 —— 正好压在门槛上，整条通路被掐死，
// 三个人声灯效全程不亮。**标定必须用真实素材，合成的只配当阴性对照。**
//
// 密集失真编曲里的人声仍然抓不到，实测（tools/lamp_calib.cpp，60s）：
//
...
struct VocalConfig {
    ...
    // 那边改了这里应当自动跟上。
    //
    // ⚠️ **下界是 200 不是 300。** 教科书的语音带是 300–3400Hz，照抄的话
    // 第 3 段（215–301Hz）整段被切掉 —— 而大量元音的第一共振峰和女声基频
    // 就在那里。实测（见下表）：下界 300 时中文语音只有 9% 的帧过阈，
    // 改成 200 之后是 57%，而纯鼓的阴性对照仍然是 0。
    float formant_lo_hz = 200.0f;
    ...
    //   能量全在共振峰带（纯人声） 0.997
    //
    // ⚠️ **0.62 是拿合成素材标的，对真实音乐是错的。** 合成的「纯人声」
    // 占比 0.997，看着离 0.62 很远；而真实音乐是 0.624 —— **正好压在门槛上**，
    // 门输出 0.011，等于把整条通路掐死。这是用户报「人声灯效完全不亮」
    // 之后拿真实音频量出来的。
    //
    // 改成 0.50，也就是「密集混音」那条参考线：高于它才说明能量真的
    // 往共振峰带集中了。真实音乐于是得到 (0.624-0.50)/0.50 = 0.25。
    float formant_share_min = 0.50f;

    // 1 − 打击占比 要过这条线。**0.25 不是 0.35** —— 语音的清辅音
    // （擦音、爆破音）是宽带噪声，HPSS 会把它们判成打击型，实测语音的
    // 「1−打击」只有 0.52。要求太高等于要求人只发元音。
    float harmonic_min = 0.25f;
...
inline void vocalUpdate(VocalState &s, const VocalConfig &c,
    ...
    // **几何平均，不是直接相乘。**
    //
    // 相乘保留了「三样缺一不可」（任一为 0 结果就是 0），但真实音乐上
    // 三个门各自只有 0.2–0.5，乘起来就是 0.03 —— 三个「都还行」被压成
    // 「完全没有」。实测（30 秒真实音乐）：
    //     voiced 0.465 × formant 0.011 × harm 0.249 = 0.0013
    // 三个人声灯效因此全程不亮，用户报的就是这个。
    //
    // 开三次方之后 0 仍然是 0（缺一不可这条没丢），但三个 0.3 会得到 0.3
    // 而不是 0.027。这是常见的模糊 AND 写法。
    s.raw = cbrtf(voiced * formant * harm);
