...
struct LampBridge {
  ...
  // 音源模式：0=自动（远端优先→本地→bridge），1=只用本地麦克风/3.5mm，2=只用 LAMP1 远端。
  // 强制态**不做另一侧回落** —— 否则"强制"二字没有意义；远端断流时落到 bridge 基础数据，
  // 而不是偷偷切回麦克风。见 fill()。
  uint8_t  srcMode = 0;
  // AI: end
  ...
  void fill(uint32_t now) {
    ...
    // AI: below section was generated by an AI
    if (srcMode != 1 && remoteFresh(now)) { fillFromRemote(); return; }
    if (srcMode == 2) { frame = AudioFrame{}; return; }   // 只用远端：断流就空帧，不回落麦克风
    // AI: end
...
class LampFxUsermod : public Usermod {
  public:
      ...
      if (!routeOk) {
        server.on("/lampdata", HTTP_GET, [](AsyncWebServerRequest *request) {
          ...
          int n = snprintf(buf, sizeof(buf),
            ...
            // AI: below section was generated by an AI
            // 生效源必须和 fill() 的仲裁同口径 —— 否则强制模式下仪表会报一个
            // 根本没在用的源（实测：srcMode=2 断流时仍显示 local）。
            (bridge.srcMode != 1 && bridge.remoteFresh(millis())) ? 2
              : (bridge.srcMode == 2 ? 0
              : ((localFrameMs && millis() - localFrameMs < 100) ? 1 : 0)),
            // AI: end
    ...
    // AI: below section was generated by an AI
    // 音源三态开关：主页顶栏发 {"lamp":{"src":0|1|2}} 到 /json/state。
    // 只在值真的变了才落盘 —— 用户手动切换很稀疏，不会磨损 flash。
    void addToJsonState(JsonObject &root) override {
      JsonObject u = root[F("lamp")];
      if (u.isNull()) u = root.createNestedObject(F("lamp"));
      u[F("src")] = bridge.srcMode;
    }

    void readFromJsonState(JsonObject &root) override {
      JsonObject u = root[F("lamp")];
      if (u.isNull()) return;
      if (!u[F("src")].is<int>()) return;
      int m = u[F("src")].as<int>();
      if (m < 0 || m > 2 || (uint8_t)m == bridge.srcMode) return;
      bridge.srcMode = (uint8_t)m;
      // 不在这里直接写文件系统：readFromJsonState 跑在 AsyncTCP 上下文，阻塞的 LittleFS
      // 写入不该放在那儿。置标志让主循环落盘 —— 与 wled.cpp 自身的做法一致。
      configNeedsWrite = true;
    }

    void addToConfig(JsonObject &root) override {
      JsonObject top = root[F("LampFx")];
      if (top.isNull()) top = root.createNestedObject(F("LampFx"));
      top[F("src")] = bridge.srcMode;
    }
    ...
    bool readFromConfig(JsonObject &root) override {
      ...
      int m = top[F("src")] | 0;
      bridge.srcMode = (m >= 0 && m <= 2) ? (uint8_t)m : 0;
      return true;
    }
    // AI: end
